...
export default function EntrepreneursWithoutTraction() {
    ...
    return (
        <section className='component w-full sm:w-auto sm:min-w-[360px]'>
            <div className="flex justify-between items-center">
                <h2 className='px-4 font-semibold text-lg my-4'>Entrepreneurs without Traction</h2>
                                        ...
                                        Days.map((day) => {
                                            return (
                                                <td key={day.date} className="text-white px-6 py-4">
                                                    {day.values.AmountByCountryWithoutTraction[country as keyof DayAmountRegistered['values']['AmountByCountryTotal']]}
                                                </td>
                                            )
                                        })
                                    }
                        ...
                        <tr key='Total' className='border-b bg-gray-800 border-gray-700'>
                            <td className='text-white font-semibold px-6 py-4'>Total</td>
                            {
                                Days.map((day) => {
                                    return (
                                        <td key={day.date} className="text-white px-6 py-4">
                                            {Object.values(day.values.AmountByCountryWithoutTraction).reduce((acc, curr) => acc + curr, 0)}
                                        </td>
                                    )
                                })
                            }
